  ...
  return new Promise((finalResolve, finalReject) => {
    this.git.getProjectList().then(repos => {
      ...
      Promise.all(extractPromises).then(() => {
        const tempName = TEMP_PATH.split('/').splice(-1)
        const jsonPath = getJsonByPath('../../' + tempName)
        fse.removeSync(TEMP_PATH);
        finalResolve(JSON.stringify(jsonPath[tempName]))
        callback && callback(JSON.stringify(jsonPath[tempName]));
      }).catch(error => {
        fse.removeSync(TEMP_PATH);
        console.log(error)
        finalReject(error)
